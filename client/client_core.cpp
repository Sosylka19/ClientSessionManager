...
void talk_to_server::connect(ip::tcp::endpoint &ep)
{
    socket_.connect(ep);
}
...
const std::string& talk_to_server::username() const
{
    return username_;
}
...
void talk_to_server::process_request()
{
...
void talk_to_server::on_handshake()
{
...
size_t talk_to_server::read_complete(const boost::system::error_code &error, size_t bytes)
{
    if (error) return 0;
    already_read_ = bytes;
